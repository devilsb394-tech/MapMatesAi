...
export default function ChatMap({ userLat, userLng, options, activeOptionId, onMapClick }: ChatMapProps) {
  ...
  // Default coordinate: Lahore if user location coordinates are not supplied
  const defaultLat = 31.5204;
  const defaultLng = 74.3587;
  const curLat = userLat && !isNaN(userLat) ? userLat : defaultLat;
  const curLng = userLng && !isNaN(userLng) ? userLng : defaultLng;
