...
export const MapVibe = forwardRef<MapVibeHandle, MapVibeProps>((props, ref) => {
  ...
  // --- DETECT ROUTING REDIRECT FROM MAPMATES AI ---
  useEffect(() => {
    ...
    const destName = params.get("destination") || "Target";

    if (action === "route" && latStr && lngStr) {
      const lat = parseFloat(latStr);
      const lng = parseFloat(lngStr);
      if (!isNaN(lat) && !isNaN(lng)) {
        console.log("URL ACTION: Automatically starting route to", destName, lat, lng);
        setTimeout(() => {
          safeFlyTo(lat, lng, 19);
          startNavDestiny({ lat, lng });
        }, 1500);
  ...
  }, [auth.currentUser, userProfile?.location]);
