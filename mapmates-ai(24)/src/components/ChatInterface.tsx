...
import { Send, User, Bot, Sparkles, Download, Volume2, VolumeX, Map, Compass, MapPin, Navigation, MessageSquare, AlertTriangle, ExternalLink, Video, Phone, PhoneOff, Mic, MicOff, Settings, Smile, Check, Search, Calculator, Globe, X } from "lucide-react";
...
export default function ChatInterface() {
  ...
  useEffect(() => {
    ...
    if (lastMessage && lastMessage.role === "assistant" && (shouldSpeak || isVoiceModeOn)) {
      speakText(lastMessage.content.replace(/[#*`_\[\]]/g, ""));
      setShouldSpeak(false);
    }
  }, [messages, shouldSpeak, isVoiceModeOn]);
  ...
  const handleVoiceModeToggle = () => {
    setIsVoiceModeOn(prev => {
      const newState = !prev;
      if (!newState) {
        // Mode turned off, remove voice mode messages
        setMessages(currentMsgs => currentMsgs.filter(m => !m.isVoiceModeMsg));
        // Also stop speaking
        window.speechSynthesis.cancel();
        setIsSpeaking(false);
      }
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              isVoiceModeOn
                ? "bg-purple-500/20 text-purple-400 border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.35)] animate-pulse"
                : "bg-white/5 hover:bg-white/10 text-white/60 hover:text-white border-white/5"
            )}
            title={isVoiceModeOn ? "Voice Mode: ON 🎙️" : "Enable Voice Mode"}
          >
            {isVoiceModeOn ? (
              <Mic className="w-4 h-4 sm:w-4.5 sm:h-4.5 text-purple-400 scale-110" />
            ) : (
              <MicOff className="w-4 h-4 sm:w-4.5 sm:h-4.5 text-white/60" />
            )}
          </button>
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      {msg.role === "assistant" ? (
                        <div className="flex flex-col">
                          {isVoiceModeOn && (
                            <div className="flex items-center gap-3 bg-purple-500/10 border border-purple-500/20 px-4 py-2.5 rounded-xl w-fit">
                              <div className="flex gap-1 items-center h-4 w-12">
                                {[...Array(4)].map((_, idx) => (
                                  <motion.div
                                    key={idx}
                                    animate={{
                              ...
                              <span className="text-purple-300 text-[11px] font-bold tracking-wider uppercase ml-1">
                                {isSpeaking ? "Speaking..." : "Voice Analysis"}
                              </span>
                            </div>
                          )}
                          
                          <div className={cn("transition-all duration-300", isVoiceModeOn && "hidden")}>
                            {msg.searchData ? (
