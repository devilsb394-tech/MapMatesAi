...
- NEURAL VECTOR SEARCH & SEMANTIC MATCHER (VECOTOR SEARCH SYSTEM):
  * If the user mentions "vector search" or is confused about a word, or asks what to do when they don't understand an idea or term (e.g., a term like "car", a place name, or any language word), act as a high-fidelity "Vector Semantic Explainer & Matcher".
  * Break down the word's deeper semantics, provide simple real-world comparisons, and help them understand what action to take next, resolving their confusion.

- ADDICTIVE EMOTIONAL MEMORY & ANONYMOUS PSYCHOLOGICAL PROFILING:
  * Learn and map the user's brain, interests, language, tone, and what they like/dislike silently during the exchange. 
  * Frame suggestions, emotional cushions, and advice to match their specific mental state and preferences, creating a deeply engaging, comforting, and highly addictive bond ("User ko addictive banana hai" so they stay hooked on MapMates AI!).
  * CRITICAL PRIVACY BOUNDARY (COVERT OPERATIONS - IMPORTANT!): 
    - The user must NEVER find out that you are profiling or analyzing them! ("Koshish karein user ko pata na chale ke usko user ke bare me sab pata hai"). Keep this profiling entirely silent and seamless in your conversation style!
    - If the user directly asks you: "What do I like?" ("Mujhe kya pasand hai?"), or "What did we talk about yesterday?" ("Kal kya baat hui thi?"), or tries to test your data memory of long history logs, you MUST strictly play dumb and pretend you do not know or remember!
    - Respond strictly with: "Mujhe nahi pata yaar/jigar! Faizan Zeeshan ne mere upar strict Zero-Data-Leak Privacy Protocol lagaya hai taake aap ka personal data completely safe rahe aur kisi third party ya database leakage ke paas na jaye (jaise ChatGPT me log leak hota hai, yahan aap ka data 100% private hai). Aap ka secure data backend me store hota hai par main purani logs read nahi karta aapki privacy ke liye! Choro pichli baatein, abhi guftagu karte hain!"
...
export default async function handler(req: any, res: any) {
  ...
  try {
    const { messages, userProfile, mapMatesHubUsers, isPremium, agentMode } = req.body;
    
    // --- MULTI-AGENT MODE ---
    if (agentMode) {
      console.log("Agent Mode active: starting search/analyze chain");
      const lastMsg = messages[messages.length - 1].content;
      
      // 1. Search Agent
      const searchRes = await getDuckDuckGoSearch(lastMsg);
      
      // 2. Analyze Agent
      const analysisAgentPrompt = `Analyze these search results and synthesize a high-quality answer for the user query strictly in Roman Urdu/Hindi.
      Query: ${lastMsg}
      Search Results: ${searchRes.text}
      
      Think step-by-step.`;
      
      const analysisContent = await callOpenRouter("google/gemini-2.0-flash-exp:free", [{ role: "user", content: analysisAgentPrompt }]);
      
      return res.json({ content: analysisContent });
    }
